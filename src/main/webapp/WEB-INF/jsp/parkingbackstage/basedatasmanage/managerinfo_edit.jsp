...
<html lang="en">
	<head>
		...
		<script type="text/javascript" src="system/js/systemjs/managerinfo_edit.js"></script>
		
		<script type="text/javascript">
			$(function(){
				var allParkInfo = ${allParkInfo} ;
				var parkManagerInfo = ${parkManagerInfo} ;
				for(var i = 0 ; i < allParkInfo.datas.length ; i ++){
			 	...
			 	$("#parkInfoId").val(parkManagerInfo.datas.parkingInfoId) ; 
				$("#managerName").val(parkManagerInfo.datas.parkingManagerName) ;
				 $("#managerPhone").val(parkManagerInfo.datas.parkingManagerPhone) ;
				 $("#oldPhone").val(parkManagerInfo.datas.parkingManagerPhone) ;
				 $("#parkingManagerId").val(parkManagerInfo.datas.parkingManagerId) ;
				 $("#parkingManagerPsd").val(parkManagerInfo.datas.parkingManagerPsd) ;
				 $("#parkingManagerActiveMark").val(parkManagerInfo.datas.parkingManagerActiveMark) ;
			}) ;
		</script>
	</head>
<body>
		<div id="zhongxin" style="margin-top:35px">
		<input type="hidden" id="parkingManagerId"/>
		<input type="hidden" id="oldPhone"/>
		<input type="hidden" id="parkingManagerPsd"/>
		<input type="hidden" id="parkingManagerCreateTime"/>
		<input type="hidden" id="parkingManagerActiveMark"/>
		<table id="table_report" class="table table-striped table-bordered table-hover">
			...
			<tr>
				<td style="text-align: center;" colspan="10">
					<a class="btn btn-mini btn-primary" onclick="resetPsw();">重置密码</a>
